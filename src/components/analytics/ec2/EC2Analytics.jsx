...
function EC2Analytics() {
    ...
    const fetchInstances = async () => {
        try {
            ...
            const res = await axiosClient.get(
                `/analytics/ec2/instances/${from}/${to}`
            );
            ...
            const transformed = json.data.instances.map(inst => {
                const dailyMetrics = (inst.active_dates || []).map(d => ({
                    date: d.date,
                    cpu: Number(d.avg_cpu_utilization) || 0,
                    cost: Number(d.approx_cost) || 0,
                    isAwsConsole: d.turned_on_by_aws_console || false
                }));
                ...
                return {
                    ...
                    avgCpu: Number(inst.avg_cpu_utilization) || 0,
                    approxCost: Number(inst.total_cost) || 0,
                    activeDays: inst.total_active_dates || 0,
                    memoryGb: Number(inst.memory_gib) || 0,

                    dailyMetrics,

                    // ✅ ADD THIS
                    isAwsConsole: dailyMetrics.some(d => d.isAwsConsole)
                };
            });

            setAllInstanceData(transformed);
    ...
    const summaryStats = useMemo(() => ({
        ...
        avgCpu: instanceData.length ? (instanceData.reduce((s, c) => s + c.avgCpu, 0) / instanceData.length).toFixed(2) : 0,
    }), [instanceData]);
    ...
    const fetchHistory = async (instanceId, date = null) => {
        try {
            const url = date
                ? `/analytics/ec2/instances/${instanceId}/metrics/history/hourly/${date}`
                : `/analytics/ec2/instances/${instanceId}/metrics/history/daily`;
            ...
            const metrics = (res.data.data.metrics || []).map(m => ({
                ...
                displayDate: m.date ? new Date(m.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : (m.timestamp ? new Date(m.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : ''),
                displayHour: m.timestamp ? m.timestamp.split(' ')[1].substring(0, 5) : null,
                timestamp: m.timestamp,
                cpu: Number(m.avg_cpu_utilization || m.cpu) || 0,
                cost: Number(m.approx_cost || m.cost || 0) || 0,
                isAwsConsole: m.turned_on_by_aws_console || false
            }));

            return metrics;
        } catch (err) {
            console.error(`Failed to fetch history for ${instanceId}`, err);
            return [];
        }
    };

    const handleGraphClick = async (instance) => {
        setLoadingTrend(true);
        const metrics = await fetchHistory(instance.instanceId);
        setSelectedInstanceForTrend({
            ...instance,
            db_identifier: instance.instanceName, // Map for RDSGraphModal title
            metrics
        });
    ...
    const metricCards = [
        ...
        { label: 'Total Cost', value: `$${summaryStats.totalCost.toFixed(2)}`, icon: DollarSign, color: '#10b981', bg: 'rgba(16,185,129,0.1)', progress: 68 },
            ...
            <div className="ec2-analytics-content">
                                        ...
                                        <div className="instance-stats-row">
                                            ...
                                            <div
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    e.stopPropagation();
                                                    const rawDates = (instance.dailyMetrics || [])
                                            ...
                                            <div className="instance-stat-item">
                                                ...
                                                <div className="isi-value">${instance.approxCost.toFixed(2)}</div>
                ...
                {instanceData.length > 0 && (
                    <ComparisonTable
                        ...
                        gridTemplateColumns="48px minmax(0, 2fr) minmax(0, 1.2fr) minmax(0, 1.2fr) minmax(0, 1fr) minmax(0, 1.2fr) minmax(0, 1.2fr)"
                                                ...
                                                onClick={() => {
                                                    if (!is24hRange) {
                                                        setSelectedDateDetail({
                                                            ...
                                                            cpu: dateObj.cpu,
                                                            cost: dateObj.cost,
                                                            isAwsConsole: dateObj.isAwsConsole,
